...
createVolume :: forall m. Monad m => VolumeCreateRequest -> DockerT m (Either DockerError DockerVolume)
createVolume req = requestHelper POST (CreateVolumeEndpoint req) >>= parseResponse
